...
struct MovieDetailView: View {
    ...
    private func loadMovie() {
        isLoading = true
        dittoService.getMovie(by: movieId) { fetchedMovie in
            self.movie = fetchedMovie
            if let movie = fetchedMovie {
                self.editTitle = movie.title
                self.editPlot = movie.plot
                self.editYear = movie.year
                self.editPoster = movie.poster
                self.editFullplot = movie.fullplot
                self.editCountries = movie.countries.joined(separator: ", ")
            }
    ...
    private func saveChanges() {
        ...
        if !updates.isEmpty {
            dittoService.updateMovie(movie, updates: updates) { success, error in
                if success {
                    saveAlertMessage = "Movie updated successfully"
                    isEditMode = false
                    loadMovie() // Reload to get updated data
                } else {
